...
void TestStats() {
  ...
  *px = RGBAPixel(11, 11, 11, 1.0);
  ...
  cout << "AREA: " << st.GetRectangleArea(0, 0, 1, 1) << endl;
  cout << "done." << endl;
...
void TestImgTreeBuildRender() {
  ...
  cout <<"Read Image from File Successfully!" << endl; 
...
void TestImgTreeFlipHorizontal() {
  ...
  cout << "Calling Copy...";
  PNG newImg = PNG(960, 540);
  ImgTree copyTree = ImgTree(newImg);
  copyTree.Copy(mytree);
  cout << "done." << endl;

  cout << "Saving copied tree into PNG file...";
  PNG copyOutput = copyTree.Render();
  copyOutput.writeToFile("images/kkkk-kmnn-copy-960x540.png");
  cout << "done." << endl;
...
void TestImgTreeCountLeavesPrune() {
  ...
  cout << "Rendering pruned ImgTree that is Flipped...";
  ImgTree newTree(inputimg);
  newTree.FlipHorizontal();
  newTree.Prune(85, 0.02);
  cout << "done." << endl;

  cout << "Saving rendered flip-prune image to file...";
  PNG newoutput = newTree.Render();
  newoutput.writeToFile("images/kkkk-kmnn-prune-flip-pct85-tol002-960x540.png");
  cout << "done." << endl;
