...
PNG ubcify(PNG image) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      double yel_dist = colordist(*pixel, RGBAPixel(247, 184, 0)); //adreference
      double blu_dist = colordist(*pixel, RGBAPixel(12, 35, 68));

      if (yel_dist <= blu_dist) {
  ...
  for (unsigned x = 0; x < secondImage.width(); x++) {
    for (unsigned y = 0; y < secondImage.height(); y++) {
      ...
      if (secondPixel->r == 255 && secondPixel->g == 255 && secondPixel->b == 255) {
        firstPixel->r = min(255, firstPixel->r + 40);
        firstPixel->g = min(255, firstPixel->g + 40);
        firstPixel->b = min(255, firstPixel->b + 40);
      }
