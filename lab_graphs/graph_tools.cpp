    ...
    while (!q.empty()) {
        ...
        for (size_t i = 0; i < adj.size(); ++i) {
            if (graph.getVertexLabel(adj[i]) == "UNEXPLORED") {
                graph.setVertexLabel(adj[i], "VISITED");
                q.push(adj[i]);
                if (vertices.find(adj[i]) != start) {
                    vertices.setunion(start, v);
                    graph.setEdgeLabel(v, adj[i], "MST");
                }
