...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    string word1c = word1;
    std::transform(word1c.begin(), word1c.end(), word1c.begin(), ::toupper);

    auto lookup1 = dict.find(word1c);
    if (lookup1 != dict.end()) {
        string word2c = word2;
        std::transform(word2c.begin(), word2c.end(), word2c.begin(), ::toupper);
        auto lookup2 = dict.find(word2c);
        if (lookup2 != dict.end()) {
