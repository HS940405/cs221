...
RGBAPixel* myFavoriteColor(int blueval) {
    //RGBAPixel pixel(64, 191, blueval); //local variable -> no pixel exists anymore as the compiler leave this function
    //return &pixel; //returning pointer(address) of pixel
...
void sketchify(std::string inputFile, std::string outputFile) {
    ...
    //using NULL or nullptr does not assign pointer to original (points nothing)
    ...
    for (unsigned y = 1; y < height - 1; y++) {
        for (unsigned x = 1; x < width - 1; x++) {
            ...
            RGBAPixel* currOutPixel = (*output).getPixel(x, y);
                                      //white image we got -> it's output
            ...
            if (score > 0.3) {
                ...
                *currOutPixel = *myPixel;
                //draw picture!!!!! 
                //currOutPixel = myPixel only chages allocation of 'currOutPixel' to myPixel, and leave the pixel to be changed as unchanged
                //to change color of currOutPixel, the value of currOutPixel should be changed to that of myPixel
            }
        }
    }
